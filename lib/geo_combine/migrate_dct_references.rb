module GeoCombine
  class MigrateDctReferences
    def initialize(metadata)
      @metadata = metadata
    end

    def migrate
      @metadata['downloads_sm'] = download_references unless download_references.empty?
      @metadata['metadata_sm'] = metadata_references unless metadata_references.empty?
      @metadata['webservices_sm'] = webservice_references unless webservice_references.empty?
      @metadata.delete('layer_id_s')
      @metadata.delete('dct_references_s')
      @metadata
    end
    ...
    private
      ...
      def references
        return {} unless @metadata['dct_references_s']
        @references ||= JSON.parse(@metadata['dct_references_s'])
      end
      ...
      def webservice_references
        references.collect do |k, v|
          ...
          next unless type
          layer_id = @metadata['layer_id_s'] if ['WMS', 'WFS'].include? type
          {
            'type' => type,
            'url' => v,
            'layerId' => layer_id,
